...
def ingest_shell_bms(df):
    ...
    # --- voltage ---
    if "Pack Vol" in df.columns:
        df["voltage"] = pd.to_numeric(df["Pack Vol"], errors="coerce")

    # --- current ---
    if "Curent" in df.columns:
        df["current"] = pd.to_numeric(df["Curent"], errors="coerce")
    ...
    # --- optional extras (NO logic change) ---
...
def strong_sanitize(df):
    ...
    df = df.copy()
    with st.expander("🔍 Raw data (verification only)", expanded=False):
...
def main():
    ...
    df_raw["t"] = df_raw.index  # elapsed time (seconds)
    ...
    with tab_ai:
        ...
        fig_h = px.line(df_out, x="t", y="battery_health_score", labels={"t":"Elapsed time (s)","battery_health_score":"Health Score"}, title="Health Score Over Time")
        safe_plot(fig_h, key="ai_health_timeline")
        fig_vv = px.line(df_out, x="t", y="voltage_var", labels={"t":"Elapsed time (s)","voltage_var":"Voltage Variance"}, title="Voltage Variance")
        safe_plot(fig_vv, key="ai_voltage_var")
        fig_soc_ai = px.line(df_out, x="t", y="soc", labels={"t":"Elapsed time (s)","soc":"SOC (%)"}, title="SOC Trend")
        safe_plot(fig_soc_ai, key="ai_soc_chart")
        fig_cur = px.line(df_out, x="t", y="current", labels={"t":"Elapsed time (s)","current":"Current (A)"}, title="Current Flow Over Time")
    ...
    with tab_trad:
        ...
        fig_v = px.line(df_out, x="t", y="voltage", labels={"t":"Elapsed time (s)","voltage":"Voltage (V)"}, title="Voltage Over Time")
        safe_plot(fig_v, key="trad_voltage_chart")
        fig_t = px.line(df_out, x="t", y="temperature", labels={"t":"Elapsed time (s)","temperature":"Temperature (°C)"}, title="Temperature Over Time")
        safe_plot(fig_t, key="trad_temp_chart")
        fig_soc_trad = px.line(df_out, x="t", y="soc", labels={"t":"Elapsed time (s)","soc":"SOC (%)"}, title="SOC Over Time (Traditional BMS)")
