...
def main():
    ...
    with tab_ai:
        ...
        fig_h = px.line(df_out, x='t_index', y="battery_health_score", labels={"time":"Time","battery_health_score":"Health Score"}, title="Health Score Over Time")
        safe_plot(fig_h, key="ai_health_timeline")
        fig_vv = px.line(df_out, x='t_index', y="voltage_var", labels={"time":"Time","voltage_var":"Voltage Variance"}, title="Voltage Variance")
        safe_plot(fig_vv, key="ai_voltage_var")
        fig_soc_ai = px.line(df_out, x='t_index', y="soc", labels={"time":"Time","soc":"SOC (%)"}, title="SOC Trend")
        safe_plot(fig_soc_ai, key="ai_soc_chart")
        fig_cur = px.line(df_out, x='t_index', y="current", labels={"time":"Time","current":"Current (A)"}, title="Current Flow Over Time")
    ...
    with tab_trad:
        ...
        fig_v = px.line(df_out, x='t_index', y="voltage", labels={"time":"Time","voltage":"Voltage (V)"}, title="Voltage Over Time")
        safe_plot(fig_v, key="trad_voltage_chart")
        fig_t = px.line(df_out, x='t_index', y="temperature", labels={"time":"Time","temperature":"Temperature (°C)"}, title="Temperature Over Time")
        safe_plot(fig_t, key="trad_temp_chart")
        fig_soc_trad = px.line(df_out, x='t_index', y="soc", labels={"time":"Time","soc":"SOC (%)"}, title="SOC Over Time (Traditional BMS)")
