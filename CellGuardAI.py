#CellGuardAI.pyimport streamlit as st
import pandas as pd
import numpy as np
...
def estimate_rul_proxy(df, stability, confidence):
    """Conservative RUL proxy based on stability decay and confidence.
    Returns qualitative RUL band instead of numeric fantasy."""
    if confidence < 40:
...
def build_pack_summary(df, health, stability, confidence, rul_band):
    """Create a fleet-ready summary dictionary."""
    return {
...
def export_snapshot(summary):
    ...
    """Estimate which signal contributes most to instability."""
...
def compute_confidence_score(df):
    """Confidence based on data completeness & stability."""
    completeness = df.notna().mean().mean()  # fraction of valid data
    variance_penalty = (
...
def scenario_replay(df, idx):
    """Return dataframe state at a specific index."""
